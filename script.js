...
function renderProducts(){const q=$("search").value.toLowerCase().trim(),cat=$("categoryFilter").value,sort=$("sort").value;let list=products.filter(p=>(cat==="All"||p.category===cat)&&(!q||[p.name,p.brand,p.category].join(" ").toLowerCase().includes(q)));if(sort==="low")list.sort((a,b)=>a.price-b.price);if(sort==="high")list.sort((a,b)=>b.price-a.price);if(sort==="rating")list.sort((a,b)=>(b.rating||0)-(a.rating||0));$("resultInfo").textContent=`Showing ${list.length} of ${products.length} products`;$("productGrid").innerHTML=list.length?list.map(p=>`<article class="product-card">${p.badge?`<span class="badge ${p.badge==="NEW"?"green":""}">${esc(p.badge)}</span>`:""}<div class="product-image">${imageHtml(p)}</div><div class="product-body"><h3>${esc(p.name)}</h3><div class="rating">${stars(p.rating)} <span>${p.rating||0} · ${p.reviews||0} reviews</span></div><div class="price-wrap">${priceHtml(p)}</div><div class="label">${esc(p.brand||"")} · ${esc(p.category||"")}</div><div class="actions">
      ${p.amazon_url
        ? `<a class="buy-now" href="${esc(p.amazon_url)}" target="_blank" rel="nofollow sponsored noopener">Buy Now ↗</a>`
        : `<button class="primary" type="button" onclick="openModal(${p.id})">View details</button>`}
      <button type="button" onclick="openModal(${p.id})">View details</button>
    </div></div></article>`).join(""):`<div style="grid-column:1/-1;text-align:center;padding:40px;background:#fff;border-radius:14px">No products found.</div>`;document.querySelectorAll("[data-view]").forEach(b=>b.onclick=()=>showProduct(b.dataset.view));document.querySelectorAll("[data-enquire]").forEach(b=>b.onclick=()=>enquire(b.dataset.enquire))}
function showProduct(id){const p=products.find(x=>String(x.id)===String(id));if(!p)return;$("modalContent").innerHTML=`<div class="detail"><div class="detail-image">${imageHtml(p)}</div><div><span class="kicker">${esc((p.category||"").toUpperCase())}</span><h2>${esc(p.name)}</h2><div class="rating">${stars(p.rating)} <span>${p.rating||0}/5 · ${p.reviews||0} reviews</span></div><div class="price">${money(p.price)}</div><p>${esc(p.description||"")}</p><h3>Key specifications</h3><ul class="specs">${(p.specs||[]).map(s=>`<li>${esc(s)}</li>`).join("")}</ul><div class="detail-actions">
      ${p.amazon_url ? `<a class="buy-now large" href="${esc(p.amazon_url)}" target="_blank" rel="nofollow sponsored noopener">Buy Now ↗</a>` : `<span class="no-link">Buy Now link not added yet</span>`}
    </div></div></div>`;$("modal").style.display="block";document.body.style.overflow="hidden";$("modalWa").onclick=()=>enquire(p.id)}
